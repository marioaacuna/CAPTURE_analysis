...
%}
    ...
    elseif isempty(fig)
        error('export_fig:no_fig','No figure found');
    end
    ...
    if cls
        ...
        if ~isequal(get(fig, 'Type'), 'figure')
            error('export_fig:no_axes','Handle must be that of a figure, axes or uipanel');
        end
    ...
    try
        ...
        if isbitmap(options)
            if abs(options.bb_padding) > 1
                ...
                error('export_fig:bad_padding_value','For bitmap output (png,jpg,tif,bmp) the padding value (-p) must be between -1<p<1')
            end
            ...
            try
                ...
                try
                    ...
                    if exist(pdf_nam_tmp, 'file')
                        error('export_fig:no_pdf', ['Could not create ' pdf_nam ' - perhaps the folder does not exist, or you do not have write permissions']);
                    else
                        error('export_fig:no_eps','Could not generate the intermediary EPS file.');
                    end
                end
            catch ex
                % Delete the eps
    ...
    % Restore recycle option
    recycle(oldRecycle);
    if ~options.append
        % Print clickable link to file
        EXT = [options.pdf options.eps options.png options.tif options.jpg options.bmp];
        EXTname = {'pdf' 'eps' 'png' 'tif' 'jpg' 'bmp'};
        % printFilePath([options.name '.' EXTname{EXT}], [], 0)
        fprintf([options.name '.' EXTname{EXT},'\n'], [], 0)
        ...
        elseif ischar(varargin{a}) && ~isempty(varargin{a})
            if varargin{a}(1) == '-'
                switch lower(varargin{a}(2:end))
                    ...
                    case 'update'
                        ...
                        try
                            ...
                            error('export_fig:cant_download','Could not download %s into %s\n',zipFileName,targetFileName);
                        end
                        ...
                        try
                            ...
                            error('export_fig:cant_unzip','Could not unzip %s\n',targetFileName);
                        end
                    case 'nofontswap'
                    ...
                    otherwise
                        try
                            ...
                            elseif strcmpi(varargin{a}(1:2),'-c')
                                ...
                                if numel(vals)~=4
                                    wasError = true;
                                    error('export_fig:bad_crop_value','option -c cannot be parsed: must be a 4-element numeric vector');
                                end
                                ...
                                if ~isscalar(val) || isnan(val)
                                    wasError = true;
                                    error('export_fig:bad_option','option %s is not recognised or cannot be parsed', varargin{a});
                                end
                                switch lower(varargin{a}(2))
                                    case 'm'
                                        ...
                                        if val <= 0
                                            wasError = true;
                                            error('export_fig:bad_maginification_value','Bad magnification value: %g (must be positive)', val);
                                        end
                                        options.magnify = val;
                                    case 'r'
                            ...
                            if wasError  % intentional raise
                                ...
                                error('export_fig:bad_input',['Unrecognized export_fig input option: ''' varargin{a} '''']);
                            end
                        end
                end
            else
                ...
                if ~isempty(p)
                    ...
                    if ~exist(p,'dir'),  error('export_fig:no_folder',['Folder ' p ' does not exist!']);  end
...
function eps_remove_background(fname, count)
    ...
    fh = fopen(fname, 'r+');
    if fh == -1
        error('export_fig:cant_open_file','Not able to open file %s.', fname);
    end
...
function add_bookmark(fname, bookmark_text)
    ...
    fh = fopen(fname, 'r');
    if fh == -1
        error('export_fig:no_file','File %s not found.', fname);
    end
    try
        fstrm = fread(fh, '*char')';
    catch ex
    ...
    fh = fopen(fname, 'w');
    if fh == -1
        error('export_fig:cant_write','Unable to open %s for writing.', fname);
    end
    try
