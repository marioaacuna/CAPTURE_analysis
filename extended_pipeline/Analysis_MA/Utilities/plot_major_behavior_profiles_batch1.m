...
ylabel('Conditions');
title('Behavioral Timeline by Condition (30 min @ 100Hz)');
...
hold on;
plot([x_pos, x_pos + scale_bins], [y_pos, y_pos], 'k-', 'LineWidth', 3);
text(x_pos + scale_bins/2, y_pos + 1, '200 s', 'HorizontalAlignment', 'center', 'FontWeight', 'bold');
...
set(gca, 'XTick', bar_positions, 'XTickLabel', behavior_names);
ylabel('Proportion of Time');
xlabel('Behaviors');
legend(unique_conditions, 'Location', 'best');
title('Behavioral Proportions by Condition');
grid on;

%% 6. 3D Scatter Plot (Single Animal Example)
figure('Position', [200, 200, 800, 600]);

% Select first animal for example and get its actual data
animal_ids = fieldnames(all_behaviors);
example_animal = animal_ids{12};
...
spine_pos = ratception_struct.markers_preproc.SpineM(example_frame_indices, :); % Use NON-ALIGNED data for trajectory
...
for i = 1:length(behavior_names)
    behavior_mask = all_behaviors.(example_animal).(behavior_names{i});
    if length(behavior_mask) == size(spine_pos, 1)
        behavior_colors_3d(behavior_mask, :) = repmat(behavior_colors(i, :), sum(behavior_mask), 1);
    end
end

% Handle frames with no dominant behavior (black)
no_behavior_mask = sum(behavior_colors_3d, 2) == 0;
behavior_colors_3d(no_behavior_mask, :) = 0.3;

scatter3(spine_pos(:,1), spine_pos(:,2), spine_pos(:,3), 10, behavior_colors_3d, 'filled', 'MarkerEdgeColor', 'k');
xlabel('X Position (mm)');
ylabel('Y Position (mm)');
zlabel('Z Position (mm)');
% zlim([0 70])
title(sprintf('Behavioral Trajectory - Animal %s', clean_example_id));
grid on;
view(45, 45);
...
% legend_handles = [];
% for i = 1:length(behavior_names)
%     legend_handles(i) = scatter3(NaN, NaN, NaN, 50, behavior_colors(i, :), 'filled');
% end
% legend(legend_handles, behavior_names, 'Location', 'eastoutside');
...
for i = 1:length(behavior_names)
    ...
    if length(unique(group_labels)) > 1 && length(group_data) > 1
        ...
        fprintf('%s: F(%d,%d) = %.3f, p = %.4f\n', behavior_name, ...
                stats.df(1), stats.df(2), stats.F, p_value);
    end
end
