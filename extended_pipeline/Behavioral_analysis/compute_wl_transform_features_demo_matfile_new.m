function ML_features = compute_wl_transform_features_demo_matfile_new(coeffstruct_in, overwrite_coeff, MLmatobjfile)
...
load(MLmatobjfile, 'appendage_anglegps');
...
opts.ds = 1;
opts.samprate = 100;% GC.upsampling_to;
opts.params = struct;
opts.params.samplingFreq = 100; %GC.upsampling_to;
...
params.fps = 300;% GC.upsampling_to;
params.difforder = 10;
...
for kk = 8
    ...
    % Initialize ml_matfile structure for this iteration
    ml_matfile = struct();

    % Load only the specific data we need for this iteration
    fprintf('Loading joint angles data for group %d...\n', kk);
    load(MLmatobjfile, 'appendage_joint_angles_pcs_hipass');
    appendage_joint_angles_pcs_hipass_kk = appendage_joint_angles_pcs_hipass{kk};
    clear appendage_joint_angles_pcs_hipass; % Free memory immediately
    ...
    if size(appendage_joint_angles_pcs_hipass_kk,1) > 10*1
        ...
        %% EUCLIDEAN WAVELETS - SINGLE PRECISION
        fprintf('starting group %f euclidean \n',kk)

        % Clear joint angles data and load euclidean data
        clear appendage_joint_angles_pcs_hipass_kk
        fprintf('Loading euclidean data for group %d...\n', kk);
        load(MLmatobjfile, 'pca_score_euc_hipassclip');
        appendage_pca_score_euc_hipassclip_kk = pca_score_euc_hipassclip{kk};
        clear pca_score_euc_hipassclip; % Free memory immediately
        ...
        clear final_row_euc appendage_pca_score_euc_hipassclip_kk

        %% Store results in ml_matfile structure - SINGLE PRECISION
        ml_matfile.COEFFS_feat_wl_appendages_euc{kk} = COEFFS_feat_wl_appendages_euc_kk;
        ml_matfile.dyadic_spectrograms_score_wl_appendages_euc{kk} = dyadic_spectrograms_score_wl_appendages_euc_kk;
        ml_matfile.explained_wl_appendages_euc{kk} = explained_wl_appendages_euc_kk;

        ml_matfile.COEFFS_feat_wl_appendages{kk} = COEFFS_feat_wl_appendages_kk;
        ml_matfile.dyadic_spectrograms_score_wl_appendages{kk} = dyadic_spectrograms_score_wl_appendages_kk;
        ml_matfile.explained_wl_appendages{kk} = explained_wl_appendages_kk;
        ...
        %% Empty case - store empty arrays
        fprintf('Storing empty arrays for group %d...\n', kk);

        ml_matfile.COEFFS_feat_wl_appendages_euc{kk} = [];
        ml_matfile.dyadic_spectrograms_score_wl_appendages_euc{kk} = [];
        ml_matfile.explained_wl_appendages_euc{kk} = [];

        ml_matfile.COEFFS_feat_wl_appendages{kk} = [];
        ml_matfile.dyadic_spectrograms_score_wl_appendages{kk} = [];
        ml_matfile.explained_wl_appendages{kk} = [];
    end

    % Save to matfile with -append flag
    fprintf('Saving wavelet features to matfile...\n');
    save(MLmatobjfile, '-struct', 'ml_matfile', '-append');

    % Clear ml_matfile to free memory
    ml_matfile = struct();
end
