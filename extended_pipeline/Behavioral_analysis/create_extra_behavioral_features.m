...
framelist_true = mocapstruct.modular_cluster_properties.clipped_index{8};%intersect(mocapstruct.modular_cluster_properties.clipped_index{markersetind},framelist);
...
if numel(framelist_true)>10 %need at least 1 s of data
    ...
    mocapstruct.modular_cluster_properties.cluster_markersets{2} = [1:10];
    for ll = mocapstruct.modular_cluster_properties.cluster_markersets{2}
    ...
    appearance_features_agg = zeros(size(mocapstruct.markers_preproc.Snout(framelist_true,:),1),numel(markers_to_loop )*(numel(markers_to_loop )-1)./2);
    ...
    [COEFF, SCORE, LATENT, TSQUARED,EXPLAINED] = pca(agg_dyn(:,1:1:end)');% pca(agg_dyn(:,:)');
    ...
    for kk = 1:numel(angle_lists)
        ...
        for ll = 1:numel( angle_lists{kk})
            ...
            %% compute the wavelets coefficients
            [wavelet_coeffs,w_map,fr_wavelet]= return_wavelets(cat(2,ML_features.joint_angles.(angle_lists{kk}{ll})(1:1:end)),...
                1:numel(1:1:numel(ML_features.joint_angles.(angle_lists{kk}{ll}))),opts);
