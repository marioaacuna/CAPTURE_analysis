...
for ll = 1:numel(timescales)
    ...
    for jj=1:numchops
        for kk=1:numchops
            fprintf('chopping up distance matrix for %i and %i of %i \n',jj,kk,numchops);
    ...
    title('integrated correlation over timescale')
...
for ll = 1:numel(timescales)
    %% clustering step
    fprintf('correlation based clustering over %i points \n',size(clustered_sequences{ll},2))
    ...
    %     tic
    fprintf('Number of clusters %i doing knn reembed or not \n',num_obs);
    if num_obs<nsample
        fprintf('Clustering (no reembed needed) \n')
        Z = linkage(clustered_sequences{ll}(behsuse,:)','ward','euclidean');% 'average', 'correlation'; 'weighted', 'correlation';
    ...
    fprintf('numel clust (min 20 fr) %i for timescale %0.1f \n',  numel(goodclust),timescales(ll)*60);
    ...
    D = dendrogram(Z);
    hold on
    plot([1,length(ic)], [clustercutoff_here,clustercutoff_here], '--')
    ...
    for clusterhere = 1:numel(gcsorted)
        ...
        % figure(395+ll)
        % h1=subplot(ceil(numel(gcsorted)/5),ceil(numel(gcsorted)/3),clusterhere);
        % if isfield(analysisstruct,'xx')
        % plot_shaded_tsne_absolute_alpha(analysisstruct,normshades,[],1,h1,1);
        % end
        % axis equal
        % axis off
        % text(h1,10,10, ['Clnr = ', num2str(gcsorted(clusterhere)) ])
        % colorbar off
        %         caxis([0.01 0.25])
    end
